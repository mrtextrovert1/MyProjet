        ...
        try (var paint = new Paint()) {
            ...
            if (solved) {
                ...
                if (answer.get(0).getPos().x == answer.get(1).getPos().x) {
                    ...
                    printLine(canvas, windowCS, paint, new Vector2d(answer.get(0).getPos().x, ownCS.getMin().y), new Vector2d(answer.get(0).getPos().x, ownCS.getMax().y));
                } else if (answer.get(0).getPos().y == answer.get(1).getPos().y) {
                    printLine(canvas, windowCS, paint, new Vector2d(answer.get(2).getPos().x, answer.get(0).getPos().y), answer.get(2).getPos());
                    printLine(canvas, windowCS, paint, new Vector2d(ownCS.getMin().x, answer.get(0).getPos().y), new Vector2d(ownCS.getMax().x, answer.get(0).getPos().y));
                }else {
    ...
    public void solve() {
        ...
        for (int i = 0; i < points.size(); i++) {
            for (int j = i + 1; j < points.size(); j++) {
                ...
                double c = points.get(j).getPos().x * points.get(i).getPos().y - points.get(i).getPos().x * points.get(j).getPos().y;
                ...
                if (answer.size() == 0) {
                    ...
                    for (Point point : triangle) {
                        double length = Math.abs(a * point.getPos().x + b * point.getPos().y + c) / sqrt;
                        if (length < shortest) {
                            answer.set(2, point);
                            shortest = length;
                        }
                    ...
                    for (Point point : triangle) {
                        double length = Math.abs(a * point.getPos().x + b * point.getPos().y + c) / sqrt;
                        if (length > 0.000000000001 && length < shortest) {
