...
public class PanelControl extends GridPanel {
    ...
    public PanelControl(
        ...
        Button addToFirstSet = new Button(
                window, false, backgroundColor, PANEL_PADDING,
                6, 7, 0, 3, 3, 1, "Добавить в треугольник",
                true, true);
        ...
        Button addToSecondSet = new Button(
                window, false, backgroundColor, PANEL_PADDING,
                6, 7, 3, 3, 3, 1, "Добавить во множество\nточек",
                true, true);
        ...
        solve.setOnClick(() -> {
            if (!PanelRendering.task.isSolved()) {
                ...
                String s = "Задача решена\n" +
                        "Пересечений: " + PanelRendering.task.getAnswer().size() / 2 + "\n" +
                        "Отдельных точек: " + PanelRendering.task.getTriangle().size();
